...
use crate::models::{
    ActionEngine, ChapterSelectors, DetailsSelectors, HomeSelectors, LayoutMapping, SearchConfig,
...
mod tests {
    ...
    #[test]
    fn hybrid_example_mixes_engines() {
        let source = hybrid_example_source();
        assert_eq!(source.id, "hybrid-example");
        assert!(matches!(source.config.home.effective_engine(), ActionEngine::Rust));
        assert!(matches!(source.config.details.effective_engine(), ActionEngine::Rust));
        assert!(matches!(source.config.chapter.effective_engine(), ActionEngine::Js));
    }
}
